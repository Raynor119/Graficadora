...
def grafic():
	...
	t1=tq[3]
	tiem.close()
	#subprocess arranca un script para ejecutar la funciones de haskell que crean el archvio DatosGraficarQ.txt
	subprocess.run(['bash','graficadora_C'])
	#Lee el archvio DatosGraficarQ.txt y lo guarda en una lista que tiene por nombre texto
	archivo_text=open("DatosGraficarQ.txt","r")
	...
	plt.title('Comportamiento de la Carga del Capacitor')
