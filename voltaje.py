...
def datos (E,R,C,T):
	...
	escribir.close()
#funcion que Grafica el Voltaje 
def grafic():
	...
	t1=tq[3]
	tiem.close()
	#subprocess arranca un script para ejecutar la funciones de haskell que crean el archvio DatosGraficarV.txt
	subprocess.run(['bash','graficadora_C'])
	#Lee el archvio DatosGraficarV.txt y lo guarda en una lista que tiene por nombre texto
	archivo_text=open("DatosGraficarV.txt","r")
	...
	plt.title('Comportamiento del Voltaje de la carga del Capacitor')
